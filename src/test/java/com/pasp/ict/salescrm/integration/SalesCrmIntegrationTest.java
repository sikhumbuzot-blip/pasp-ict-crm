...
/**
 * Integration tests for the Sales CRM Application.
 * Tests complete user workflows, database operations, transaction management,
 * and security integration using @SpringBootTest and @TestMethodOrder.
 * 
...
class SalesCrmIntegrationTest {
    ...
    void setUp() {
        ...
        regularUser = createTestUser("regular", "regular@test.com", "Regular", "User", UserRole.REGULAR);
    ...
    void testCompleteSalesWorkflow() throws Exception {
        // Step 1: Create a lead
        Lead lead = salesService.createLead("Test Lead", "Test Description", 
                                          BigDecimal.valueOf(1000), testCustomer, salesUser, salesUser);
        
        assertNotNull(lead);
        assertEquals(LeadStatus.NEW, lead.getStatus());
        assertEquals("Test Lead", lead.getTitle());
    ...
    @Order(2)
    @WithMockUser(username = "sales", roles = {"SALES"})
    void testDatabaseTransactionManagement() throws Exception {
        // Create initial data
    ...
    @Test
    @Order(3)
    void testRoleBasedAccessControlIntegration() throws Exception {
    ...
    @Order(4)
    @WithMockUser(username = "sales", roles = {"SALES"})
    void testCustomerDataManagementWorkflow() throws Exception {
     ...
     * Tests that the system handles concurrent operations from multiple users correctly.
     */
    @Test
    @Order(5)
    ...
    void testMultiUserConcurrentOperations() throws Exception {
        ...
        Lead salesLead = salesService.createLead("Sales User Lead", "Description", 
                                                BigDecimal.valueOf(800), testCustomer, salesUser, adminUser);
        
        User anotherSalesUser = createTestUser("sales2", "sales2@test.com", "Sales2", "User", UserRole.SALES);
        Lead anotherLead = salesService.createLead("Another Sales Lead", "Description", 
                                                 BigDecimal.valueOf(900), testCustomer, anotherSalesUser, adminUser);

        // Verify leads are properly isolated by assigned user
        List<Lead> salesUserLeads = salesService.findLeadsByAssignedUser(salesUser);
        List<Lead> anotherUserLeads = salesService.findLeadsByAssignedUser(anotherSalesUser);

        assertEquals(1, salesUserLeads.size());
        assertEquals(salesLead.getId(), salesUserLeads.get(0).getId());
        
        assertEquals(1, anotherUserLeads.size());
        assertEquals(anotherLead.getId(), anotherUserLeads.get(0).getId());

        // Test concurrent sales operations
        SaleTransaction sale1 = salesService.convertLeadToSale(salesLead.getId(), BigDecimal.valueOf(850), 
                                                             LocalDateTime.now(), "Sale 1", salesUser);
        SaleTransaction sale2 = salesService.convertLeadToSale(anotherLead.getId(), BigDecimal.valueOf(950), 
                                                             LocalDateTime.now(), "Sale 2", anotherSalesUser);

        // Verify both sales were created successfully
        assertNotNull(sale1);
        assertNotNull(sale2);
        assertEquals(2, saleTransactionRepository.count());

        // Verify individual performance metrics are calculated correctly
        var salesUserMetrics = salesService.getIndividualPerformance(salesUser);
        var anotherUserMetrics = salesService.getIndividualPerformance(anotherSalesUser);

        assertEquals(BigDecimal.valueOf(850), salesUserMetrics.get("totalRevenue"));
        assertEquals(BigDecimal.valueOf(950), anotherUserMetrics.get("totalRevenue"));
    }
    ...
    @Order(6)
    @WithMockUser(username = "sales", roles = {"SALES"})
    void testDataIntegrityAndValidation() throws Exception {
    ...
    /**
     * Test 7: Security Integration and Audit Logging
     * Tests that security events are properly logged and audit trails are maintained.
     */
    @Test
    @Order(7)
    @WithMockUser(username = "admin", roles = {"ADMIN"})
    void testSecurityIntegrationAndAuditLogging() throws Exception {
        long initialAuditCount = auditLogRepository.count();
     ...
     * Tests system performance with larger datasets and concurrent operations.
     */
    @Test
    @Order(8)
    @WithMockUser(username = "admin", roles = {"ADMIN"})
